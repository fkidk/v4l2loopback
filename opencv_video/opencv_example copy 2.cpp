...
/*START OF VIDEO FEED INIT*/

	struct v4l2_capability vid_caps;
	struct v4l2_format vid_format;

	size_t framesize = FRAME_WIDTH * 2 + FRAME_HEIGHT;
	size_t linewidth = 0;
  ...
  if(debug)print_format(&vid_format);
	...
	vid_format.fmt.pix.colorspace = V4L2_COLORSPACE_DEFAULT;// V4L2_COLORSPACE_SRGB;
...
//      cuda_merged_stream.waitForCompletion();
      NewImg.download(result_img);
    	...
    	write(fdwr, result_img.data, framesize);
