/*
 * WORKING !!!
 * https://www.kernel.org/doc/html/v4.15/media/uapi/v4l/pixfmt-packed-rgb.html
 */
...
#define CHANNELS (3)
#define BYTES_PR_LINE (2560 * CHANNELS)
...
int open_video() {
  ...
  v.fmt.pix.bytesperline = BYTES_PR_LINE;
...
int csi() {
  ...
  cv::Point2f pts1[] = {cv::Point2f( 272,  15),
                        cv::Point2f( 990, 103),
                        cv::Point2f( 280, 691),
                        cv::Point2f(1270, 655)};
  ...
  cv::Point2f pts3[] = {cv::Point2f( 413,  33),
                        cv::Point2f(1133,  32),
                        cv::Point2f(  81, 530),
                        cv::Point2f(1037, 711)};
...
#ifdef FIND_COORDINATES
      dst_rear_img = cuda_rear_img;
      dst_front_img = cuda_front_img;
#else
...
#ifdef FIND_COORDINATES
      img = result_img;
      cv::setMouseCallback("Merged", onMouse, 0 );
